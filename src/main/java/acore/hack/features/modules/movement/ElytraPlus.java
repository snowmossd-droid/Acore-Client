...
public class ElytraPlus extends Module {
   ...
   private final Setting<ElytraPlus.AntiKick> antiKick = new Setting<>("AntiKick", ElytraPlus.AntiKick.Jitter, v -> this.mode.is(ElytraPlus.Mode.FireWork) || this.mode.is(ElytraPlus.Mode.SunriseOld));
   private final Setting<Float> xzSpeed = new Setting<>("XZSpeed", 1.55F, 0.1F, 10.0F, v -> !this.mode.is(ElytraPlus.Mode.Boost) && this.mode.getValue() != ElytraPlus.Mode.Pitch40Infinite);
   private final Setting<Float> ySpeed = new Setting<>("YSpeed", 0.47F, 0.0F, 2.0F, v -> this.mode.is(ElytraPlus.Mode.FireWork) || this.mode.getValue() == ElytraPlus.Mode.SunriseOld || this.mode.is(ElytraPlus.Mode.Packet) && this.vertical.getValue());
   private final Setting<Integer> fireSlot = new Setting<>("FireSlot", 1, 1, 9, v -> this.mode.is(ElytraPlus.Mode.FireWork));
   private final Setting<BooleanSettingGroup> accelerate = new Setting<>("Acceleration", new BooleanSettingGroup(false), v -> this.mode.is(ElytraPlus.Mode.Control) || this.mode.is(ElytraPlus.Mode.Packet));
   private final Setting<Float> accelerateFactor = new Setting<>("AccelerateFactor", 9.0F, 0.0F, 100.0F, v -> this.mode.is(ElytraPlus.Mode.Control) || this.mode.is(ElytraPlus.Mode.Packet)).addToGroup(this.accelerate);
   ...
   private final Setting<Boolean> bowBomb = new Setting<>("BowBomb", false, v -> this.mode.is(ElytraPlus.Mode.FireWork) || this.mode.getValue() == ElytraPlus.Mode.SunriseOld);
   ...
   private final Setting<Boolean> instantFly = new Setting<>("InstantFly", true, v -> this.mode.is(ElytraPlus.Mode.Boost) && !this.twoBee.getValue() || this.mode.is(ElytraPlus.Mode.Control));
   ...
   private final Setting<Float> upSpeed = new Setting<>("UpSpeed", 1.0F, 0.01F, 5.0F, v -> this.mode.is(ElytraPlus.Mode.Boost) && !this.twoBee.getValue() || this.mode.is(ElytraPlus.Mode.Control));
   private final Setting<Float> downFactor = new Setting<>("Glide", 1.0F, 0.0F, 2.0F, v -> this.mode.is(ElytraPlus.Mode.Boost) && !this.twoBee.getValue() || this.mode.is(ElytraPlus.Mode.Control));
   ...
   private final Setting<Float> minUpSpeed = new Setting<>("MinUpSpeed", 0.5F, 0.1F, 5.0F, v -> this.mode.is(ElytraPlus.Mode.Boost) && this.cruiseControl.getValue());
   ...
   private final Setting<Float> redeployInterval = new Setting<>("RedeployInterval", 1.0F, 0.1F, 5.0F, v -> this.mode.is(ElytraPlus.Mode.Boost) && !this.twoBee.getValue());
   private final Setting<Float> redeployTimeOut = new Setting<>("RedeployTimeout", 5.0F, 0.1F, 20.0F, v -> this.mode.is(ElytraPlus.Mode.Boost) && !this.twoBee.getValue());
   private final Setting<Float> redeployDelay = new Setting<>("RedeployDelay", 0.5F, 0.1F, 1.0F, v -> this.mode.is(ElytraPlus.Mode.Boost) && !this.twoBee.getValue());
   private final Setting<Float> infiniteMaxSpeed = new Setting<>("InfiniteMaxSpeed", 150.0F, 50.0F, 170.0F, v -> this.mode.getValue() == ElytraPlus.Mode.Pitch40Infinite);
   private final Setting<Float> infiniteMinSpeed = new Setting<>("InfiniteMinSpeed", 25.0F, 10.0F, 70.0F, v -> this.mode.getValue() == ElytraPlus.Mode.Pitch40Infinite);
   private final Setting<Integer> infiniteMaxHeight = new Setting<>("InfiniteMaxHeight", 200, 50, 360, v -> this.mode.getValue() == ElytraPlus.Mode.Pitch40Infinite);
   ...
   public ElytraPlus() {
      super("ElytraPlus", "Enhanced elytra flight.", Category.MOVEMENT);
   }

   @Override
   public void onEnable() {
      if (mc.player.getY() < this.infiniteMaxHeight.getValue().intValue() && this.mode.getValue() == ElytraPlus.Mode.Pitch40Infinite) {
         this.disable("Elytra's about to break!");
         mc.world.playSound(mc.player, mc.player.getX(), mc.player.getY(), mc.player.getZ(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.AMBIENT, 10.0F, 1.0F, 0L);
      }
   }
      ...
      if (elytra != -1) {
         if (mc.player.isOnGround()) {
            ...
            if (!mc.options.jumpKey.isPressed() && !mc.options.sneakKey.isPressed()) {
               ...
               mc.player.setVelocity(mc.player.getVelocity().x, -0.005F, mc.player.getVelocity().z);
            } else {
               this.acceleration = 0.0F;
               this.takeOnElytra();
            }
   ...
   private float getInfinitePitch() {
      if (mc.player.getY() < this.infiniteMaxHeight.getValue().intValue()) {
   ...
   public void onPacketSend(PacketEvent.SendPost event) {
      if (!fullNullCheck()) {
         if (event.getPacket() instanceof ClientCommandC2SPacket command && this.mode.is(ElytraPlus.Mode.FireWork) && command.getMode() == ClientCommandC2SPacket.Mode.START_FALL_FLYING) {
            this.doFireWork(false);
         }
         if (event.getPacket() instanceof PlayerInteractEntityC2SPacket p && this.mode.is(ElytraPlus.Mode.FireWork) && this.grim.getValue().isEnabled() && this.fireWorkExtender.getValue() && this.flying && this.flightZonePos != null && Criticals.getEntity(p).age < (float)this.pingTimer.getPassedTimeMs() / 50.0F) {
   ...
   public void onPacketReceive(PacketEvent.Receive e) {
      if (e.getPacket() instanceof EntityTrackerUpdateS2CPacket pac && pac.id() == mc.player.getId() && (this.mode.is(ElytraPlus.Mode.Packet) || this.mode.is(ElytraPlus.Mode.SunriseOld))) {
      ...
      if (e.getPacket() instanceof CommonPingS2CPacket && this.mode.is(ElytraPlus.Mode.FireWork) && this.grim.getValue().isEnabled() && this.fireWorkExtender.getValue() && this.flying) {
   ...
   private void doMotionPacket(EventMove e) {
      mc.player.getAbilities().flying = false;
      mc.player.getAbilities().setFlySpeed(0.05F);
      if ((!this.isBoxCollidingGround() || !this.stopOnGround.getValue()) && mc.player.getInventory().getStack(38).getItem() == Items.ELYTRA) {
         mc.player.getAbilities().flying = true;
         mc.player.getAbilities().setFlySpeed(this.xzSpeed.getValue() / 15.0F * (this.accelerate.getValue().isEnabled() ? Math.min((this.acceleration = this.acceleration + this.accelerateFactor.getValue()) / 100.0F, 1.0F) : 1.0F));
         e.cancel();
         if (mc.player.age % 3 == 0 && this.ncpStrict.is(ElytraPlus.NCPStrict.Motion)) {
            e.setY(0.0);
            e.setX(0.0);
            e.setZ(0.0);
         } else {
            ...
            e.setY(this.vertical.getValue() ? (mc.options.jumpKey.isPressed() ? this.ySpeed.getValue().floatValue() : (mc.options.sneakKey.isPressed() ? -this.ySpeed.getValue() : 0.0)) : 0.0);
            switch ((ElytraPlus.NCPStrict)this.ncpStrict.getValue()) {
               case Old:
                  e.setY(2.0E-4 - (mc.player.age % 2 == 0 ? 0.0 : 1.0E-6) + MathUtility.random(0.0, 9.0E-7));
                  break;
               case New:
            ...
            if (mc.player.isOnGround() && (this.ncpStrict.is(ElytraPlus.NCPStrict.New) || this.ncpStrict.is(ElytraPlus.NCPStrict.Motion)) && mc.player.age % 2 == 0) {
   ...
   private void doPreLegacy() {
      if (!this.twoBee.getValue() || !this.mode.is(ElytraPlus.Mode.Boost)) {
         ...
         if (!this.cruiseControl.getValue()) {
            this.height = (float)mc.player.getY();
         }
         if (this.strictTimer.passedMs(1500L) && !this.strictTimer.passedMs(2000L)) {
            ArisCore.TICK_TIMER = 1.0F;
         }
         if (!mc.player.isFallFlying()) {
            if (this.hasTouchedGround && !mc.player.isOnGround() && mc.player.fallDistance > 0.0F && this.instantFly.getValue()) {
               ArisCore.TICK_TIMER = 0.3F;
            }
            if (!mc.player.isOnGround() && this.instantFly.getValue() && mc.player.getVelocity().y < 0.0) {
               if (!this.startTimer.passedMs((long)(1000.0F * this.redeployDelay.getValue()))) {
                  return;
               }
               this.startTimer.reset();
               this.sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
   ...
   public void onRender3D(MatrixStack stack) {
      if (this.mode.is(ElytraPlus.Mode.FireWork) && this.grim.getValue().isEnabled() && this.fireWorkExtender.getValue() && this.flying && this.flightZonePos != null) {
         stack.push();
         ...
         for (int i = 0; i <= 30; i++) {
            float cos = (float)(this.flightZonePos.x - mc.getEntityRenderDispatcher().camera.getPos().x + Math.cos(i * (Math.PI * 2) / 30.0) * 95.0);
            float sin = (float)(this.flightZonePos.z - mc.getEntityRenderDispatcher().camera.getPos().z + Math.sin(i * (Math.PI * 2) / 30.0) * 95.0);
            bufferBuilder.vertex(stack.peek().getPositionMatrix(), cos, (float)(-mc.getEntityRenderDispatcher().camera.getPos().y), sin).color(Render2DEngine.injectAlpha(HudEditor.getColor(i), 255).getRGB());
            bufferBuilder.vertex(stack.peek().getPositionMatrix(), cos, (float)(128.0 - mc.getEntityRenderDispatcher().camera.getPos().y), sin).color(Render2DEngine.injectAlpha(HudEditor.getColor(i), 0).getRGB());
         }
   ...
   public void onRender2D(DrawContext context) {
      if (this.mode.is(ElytraPlus.Mode.FireWork) && this.grim.getValue().isEnabled() && this.fireWorkExtender.getValue() && this.flying && !this.pingTimer.passedMs(50000L) && this.pingTimer.passedMs(1000L)) {
         ...
         FontRenderers.sf_bold.drawCenteredString(context.getMatrices(), timeS + " seconds and " + dist + " meters left", mc.getWindow().getScaledWidth() / 2.0F, mc.getWindow().getScaledHeight() / 2.0F + 30.0F, -1);
      }
   }
      ...
      if (elytra != -1) {
         ...
         if (mc.player.fallDistance > 0.25F) {
            ...
            if (InputUtil.isKeyPressed(mc.getWindow().getHandle(), this.bombKey.getValue().getKey())) {
               ...
               mc.player.setVelocity(mc.player.getVelocity().x, mc.player.age % 2 == 0 ? 0.42F : -0.42F, mc.player.getVelocity().z);
               ...
               switch ((ElytraPlus.AntiKick)this.antiKick.getValue()) {
                  case Off:
                     ...
                     break;
                  case Jitter:
                     mc.player.setVelocity(mc.player.getVelocity().x, mc.player.age % 2 == 0 ? 0.08F : -0.08F, mc.player.getVelocity().z);
                     break;
                  case Glide:
                     mc.player.setVelocity(mc.player.getVelocity().x, -0.01F - (mc.player.age % 2 == 0 ? 1.0E-4F : 0.006F), mc.player.getVelocity().z);
               }
            }
            if (!mc.player.isSneaking() && mc.options.jumpKey.isPressed()) {
               mc.player.setVelocity(mc.player.getVelocity().x, this.ySpeed.getValue().floatValue(), mc.player.getVelocity().z);
            }
            if (mc.options.sneakKey.isPressed()) {
               mc.player.setVelocity(mc.player.getVelocity().x, -this.ySpeed.getValue(), mc.player.getVelocity().z);
            }
         }
      }
   }
   ...
   private void doBoost(EventMove e) {
      if (mc.player.getInventory().getStack(38).getItem() == Items.ELYTRA && mc.player.isFallFlying() && !mc.player.isClimbing() && !mc.player.isTouchingWater() && mc.player.isFallFlying()) {
         ...
         if (this.cruiseControl.getValue()) {
            ...
            if (this.twoBee.getValue()) {
               if (Managers.PLAYER.currentPlayerSpeed >= this.minUpSpeed.getValue()) {
                  mc.player.setPitch((float)MathHelper.clamp(MathHelper.wrapDegrees(Math.toDegrees(Math.atan2((this.height - mc.player.getY()) * -1.0, 10.0))), -50.0F, 50.0F));
               } else {
                  mc.player.setPitch(0.25F);
               }
            } else {
               double heightPct = 1.0 - Math.sqrt(MathHelper.clamp(Managers.PLAYER.currentPlayerSpeed / 1.7, 0.0, 1.0));
               if (Managers.PLAYER.currentPlayerSpeed >= this.minUpSpeed.getValue() && this.startTimer.passedMs((long)(2000.0F * this.redeployInterval.getValue()))) {
                  double pitch = -(44.4 * heightPct + 0.6);
                  double diff = (this.height + 1.0F - mc.player.getY()) * 2.0;
         ...
         if (this.twoBee.getValue()) {
            ...
            float f4 = (float)(Math.pow(Math.cos(Math.toRadians(mc.player.getPitch())), 2.0) * Math.min(1.0, rotationVec.length() / 0.4));
            ...
            if (mc.player.getPitch() > 0.0F && e.getY() < 0.0) {
               if (moveForward != 0.0F && this.startTimer.passedMs((long)(2000.0F * this.redeployInterval.getValue())) && this.redeployTimer.passedMs((long)(1000.0F * this.redeployTimeOut.getValue()))) {
                  ...
                  e.setX(e.getX() - moveForward * Math.sin(Math.toRadians(mc.player.getYaw())) * this.factor.getValue().floatValue() / 20.0);
                  e.setZ(e.getZ() + moveForward * Math.cos(Math.toRadians(mc.player.getYaw())) * this.factor.getValue().floatValue() / 20.0);
   ...
   private void doControl(EventMove e) {
      if (mc.player.getInventory().getStack(38).getItem() == Items.ELYTRA && mc.player.isFallFlying()) {
         double[] dir = MovementUtility.forward(this.xzSpeed.getValue() * (this.accelerate.getValue().isEnabled() ? Math.min((this.acceleration = this.acceleration + this.accelerateFactor.getValue()) / 100.0F, 1.0F) : 1.0F));
         e.setX(dir[0]);
         e.setY(mc.options.jumpKey.isPressed() ? this.upSpeed.getValue().floatValue() : (mc.options.sneakKey.isPressed() ? -this.sneakDownSpeed.getValue() : -0.08 * this.downFactor.getValue().floatValue()));
         e.setZ(dir[1]);
         if (!MovementUtility.isMoving()) {
            this.acceleration = 0.0F;
         }
   ...
   public void matrixDisabler(int elytra) {
      elytra = elytra >= 0 && elytra < 9 ? elytra + 36 : elytra;
      if (elytra != -2) {
         mc.interactionManager.clickSlot(mc.player.playerScreenHandler.syncId, elytra, 1, SlotActionType.PICKUP, mc.player);
         mc.interactionManager.clickSlot(mc.player.playerScreenHandler.syncId, 6, 1, SlotActionType.PICKUP, mc.player);
      }
      mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
      if (elytra != -2) {
         mc.interactionManager.clickSlot(mc.player.playerScreenHandler.syncId, 6, 1, SlotActionType.PICKUP, mc.player);
   ...
   private void doFireWork(boolean started) {
      if (!started || !((float)(System.currentTimeMillis() - this.lastFireworkTime) < this.fireDelay.getValue() * 1000.0F)) {
         if (!this.grim.getValue().isEnabled() || !this.fireWorkExtender.getValue() || !started || !this.pingTimer.passedMs(200L) || this.flightZonePos == null || !(PlayerUtility.getSquaredDistance2D(this.flightZonePos) < 7000.0F)) {
            if (!started || mc.player.isFallFlying()) {
               if (started || Managers.PLAYER.ticksElytraFlying <= 1) {
                  ...
                  if (slot == -1) {
                     ...
                     this.sendSequencedPacket(id -> new PlayerInteractItemC2SPacket(inOffhand ? Hand.OFF_HAND : Hand.MAIN_HAND, id, mc.player.getYaw(), mc.player.getPitch()));
                     ...
                     this.pingTimer.reset();
                     this.flightZonePos = mc.player.getPos();
                  }
               }
            }
         }
   ...
   private void equipElytra() {
      ...
      if (elytraSlot == -1 && mc.player.playerScreenHandler.getCursorStack().getItem() != Items.ELYTRA) {
         this.noElytra();
   ...
   private void returnChestPlate() {
      if (this.prevElytraSlot != -1 && this.prevArmorItem != Items.AIR) {
         ...
         ItemStack is = mc.player.getInventory().getStack(this.prevElytraSlot);
         ...
         n2 = n2 < 9 && n2 != -1 ? n2 + 36 : n2;
         if (mc.player.playerScreenHandler.getCursorStack().getItem() != Items.AIR) {
            clickSlot(6);
            if (this.prevElytraSlot != -1) {
               clickSlot(this.prevElytraSlot);
            }
   ...
   public void fireWorkOnPlayerUpdate() {
      ...
      boolean aboveLiquid = isAboveLiquid(0.1F) && inAir && mc.player.getVelocity().y < 0.0;
      ...
      if (this.canFly()) {
         if (!mc.player.isFallFlying() && !this.started && mc.player.getVelocity().y < 0.0) {
            this.sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_FALL_FLYING));
            this.started = true;
         }
   ...
   public void fireworkOnSync() {
      if (this.grim.getValue().isEnabled() && this.rotate.getValue()) {
         if (mc.options.jumpKey.isPressed() && mc.player.isFallFlying() && this.flying) {
            mc.player.setPitch(-45.0F);
         }
         if (mc.options.sneakKey.isPressed() && mc.player.isFallFlying() && this.flying) {
            mc.player.setPitch(45.0F);
         }
         mc.player.setYaw(MovementUtility.getMoveDirection());
      }

      if (!MovementUtility.isMoving() && mc.options.jumpKey.isPressed() && mc.player.isFallFlying() && this.flying) {
   ...
   public void fireworkOnMove(EventMove e) {
      if (mc.player.isFallFlying() && this.flying) {
         ...
         if (mc.options.jumpKey.isPressed()) {
            ...
            switch ((ElytraPlus.AntiKick)this.antiKick.getValue()) {
               case Off:
                  ...
                  break;
               case Jitter:
                  e.setY(mc.player.age % 2 == 0 ? 0.08F : -0.08F);
                  break;
               case Glide:
                  e.setY(-0.08F);
            }
   ...
   public static boolean checkGround(float f2) {
      return mc.player.getY() < 0.0 ? false : !mc.world.getBlockCollisions(mc.player, mc.player.getBoundingBox().offset(0.0, -f2, 0.0)).iterator().hasNext();
   }

   public static boolean isAboveLiquid(float offset) {
      return mc.player == null ? false : mc.world.getBlockState(BlockPos.ofFloored(mc.player.getX(), mc.player.getY() - offset, mc.player.getZ())).getBlock() instanceof FluidBlock;
   }

   public void fireworkOnEnable() {
      if (mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem() != Items.ELYTRA && mc.player.playerScreenHandler.getCursorStack().getItem() != Items.ELYTRA && InventoryUtility.getElytra() == -1) {
         this.noElytra();
      } else if (this.getFireWorks(false) == -1) {
         this.noFireworks();
      } else if (this.getFireWorks(true) == -1) {
         this.getStackInSlotCopy = mc.player.getInventory().getStack(this.fireSlot.getValue() - 1).copy();
         this.prevItemInHand = mc.player.getInventory().getStack(this.fireSlot.getValue() - 1).getItem();
      }
   }
