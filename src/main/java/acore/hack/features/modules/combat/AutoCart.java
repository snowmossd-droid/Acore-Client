...
public class AutoCart extends Module {
   ...
   private final Setting<SettingGroup> cartAuraTargets = new Setting<>("Target", new SettingGroup(false, 0), v -> this.mode.is(AutoCart.Mode.CrossBow) && this.cartAura.getValue());
   private final Setting<Boolean> cartAuraTarget = new Setting<>("Aura Target", true, v -> this.mode.is(AutoCart.Mode.CrossBow) && this.cartAura.getValue()).addToGroup(this.cartAuraTargets);
   private final Setting<Boolean> cartOtherPlayer = new Setting<>("Other Player", false, v -> this.mode.is(AutoCart.Mode.CrossBow) && this.cartAura.getValue()).addToGroup(this.cartAuraTargets);
   ...
   private void handleRefill() {
      if (this.reFill.is(AutoCart.ReFillMode.None)) {
         this.clearRefillState();
      } else if (!this.scheduledRefillSwap && mc.currentScreen == null && mc.player.playerScreenHandler == mc.player.currentScreenHandler) {
   ...
   private void performRefillSwap(int inventorySlot, int hotbarSlot) {
      if (inventorySlot != -1 && hotbarSlot >= 0 && hotbarSlot <= 8) {
         if (mc.currentScreen == null && mc.player.playerScreenHandler == mc.player.currentScreenHandler) {
      ...
      if (!fullNullCheck() && this.mode.is(AutoCart.Mode.CrossBow) && this.cartAura.getValue()) {
         if (!this.cartAuraExecuting) {
            ...
            if (popTarget != mc.player) {
               ...
               boolean isOtherPlayer = this.cartOtherPlayer.getValue() && popTarget != null && mc.player.distanceTo(popTarget) <= 6.0;
                  ...
                  if (mc.world.getBlockState(bp).isSolid() && !(bp.getY() + 1 > targetBox.minY)) {
                     ...
                     if (aboveState.isAir() || this.isRailBlock(aboveState.getBlock())) {
                        ...
                        if (!(PlayerUtility.squaredDistanceFromEyes(surfacePos) > 20.25F)) {
                           BlockHitResult blockCheck = mc.world.raycast(new RaycastContext(playerEyes, surfacePos, ShapeType.COLLIDER, FluidHandling.NONE, mc.player));
                           if ((blockCheck == null || blockCheck.getType() != Type.BLOCK || blockCheck.getBlockPos().equals(bp)) && this.isPathClearOfPlayers(playerEyes, surfacePos, target)) {
                              Vec3d cartCenter = new Vec3d(bp.getX() + 0.5, bp.getY() + 1.5, bp.getZ() + 0.5);
                              BlockHitResult damageCheck = mc.world.raycast(new RaycastContext(cartCenter, targetFeet, ShapeType.COLLIDER, FluidHandling.NONE, mc.player));
   ...
   private boolean hasFlameEnchant(ItemStack stack) {
      return mc.world == null ? false : EnchantmentHelper.getLevel(mc.world.getRegistryManager().get(Enchantments.FLAME).getEntry(Enchantments.FLAME).get(), stack) > 0;
   }
