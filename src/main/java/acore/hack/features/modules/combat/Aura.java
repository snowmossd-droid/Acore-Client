...
public class Aura extends Module {
    ...
    private boolean shouldSkipEntity(Entity entity) {
        ...
        if (entity instanceof PlayerEntity player) {
            ...
            if (ignoreNaked && player.getArmor() == 0) return true;
            // Ignore team - temporarily disabled
        }
        // Check MONSTERS (zombie, skeleton, spider, creeper, etc.)
        else if (entity instanceof HostileEntity || entity instanceof MobEntity) {
            if (!monsters) return true;
        }
        // Check ANIMALS (cow, pig, sheep, chicken, etc.)
