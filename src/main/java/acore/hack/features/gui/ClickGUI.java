...
public class ClickGUI extends Screen {
    ...
    private static final int SET_ITH   = 22;   // chiều cao mỗi dòng setting
    ...
    // settings inline – mở bên dưới module nào
        ...
        for (int c = 0; c < ACCS.length; c++) {
            ...
            if (open) {
                for (String mod : ac.items) {
                    ...
                    // render settings inline nếu đang mở
    ...
    private void renderModItem(DrawContext ctx, int mx, int my, String mod,
        ...
        if (iy >= clipY0) {
            ...
            // settings chevron bên phải (thay keybind badge)
            if (hasSet) {
    ...
    // ─── Inline Settings – hiện ngay dưới module ──────────────────
            ...
            for (int c = 0; c < ACCS.length; c++) {
                ...
                if (mx >= winX && mx < winX + WIN_W && my >= iy && my < iy + CAT_H
                    ...
                    if (!catOpen[c] && openMod != null) {
                        // đóng category → đóng settings nếu module thuộc cat này
                    ...
                    for (String mod : ac.items) {
                        ...
                        if (mx >= winX && mx < winX + WIN_W && my >= iy && my < iy + ITEM_H
                            ...
                            if (button == 0) {
                                ...
                                // right click → toggle settings panel inline
    ...
    private List<SI> buildSI(String name) {
        ...
        if (name.equals("AutoTNTcart") && rawMod instanceof AutoTNTcart m) {
            ...
            l.add(new SI("Auto Totem", m.autoPickTotem.isEnabled(),           SType.TOGGLE));
    ...
    private void applyMod(String name, SI s) {
        ...
        if (rawMod instanceof AutoTNTcart m) {
            switch (s.name) {
                ...
                case "Auto Totem"  -> m.autoPickTotem.setEnabled((boolean)s.value);
