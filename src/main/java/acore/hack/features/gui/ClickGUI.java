...
public class ClickGUI extends Screen {
    ...
    private void initSettings() {
        ...
        if (selectedMainCategory == 0 && selectedSubCategory == 0 && aura != null) {
        ...
        if (selectedMainCategory == 0 && selectedSubCategory == 1 && aura != null) {
        ...
        if (selectedMainCategory == 0 && selectedSubCategory == 2 && aura != null) {
        ...
        if (selectedMainCategory == 0 && selectedSubCategory == 3 && aura != null) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.fill(0, 40, width, 41, BORDER);
        ...
        // Left Panel
        context.fill(0, 41, 140, height, MAIN_CAT_BG);
        context.fill(140, 41, 141, height, BORDER);
        ...
        for (int i = 0; i < mainCategories.size(); i++) {
            ...
            int bgColor = selected ? MAIN_CAT_SELECTED : (hover ? MAIN_CAT_HOVER : MAIN_CAT_BG);
            context.fill(0, yOffset - 12, 140, yOffset + 20, bgColor);
            if (selected) context.fill(0, yOffset - 12, 4, yOffset + 20, ACCENT);
            context.drawText(textRenderer, cat.name, 20, yOffset, selected ? ACCENT : TEXT, true);
            yOffset += 35;
        }
        
        // Middle Panel
        context.fill(141, 41, 340, height, SUB_CAT_BG);
        context.fill(340, 41, 341, height, BORDER);
        ...
        for (int i = 0; i < subCategories.size(); i++) {
            ...
            int bgColor = selected ? SUB_CAT_SELECTED : (hover ? SETTING_HOVER : SUB_CAT_BG);
        ...
        // Right Panel
        context.fill(341, 41, width, height, SETTINGS_BG);
        context.fill(340, 41, 341, height, BORDER);
        ...
        for (SettingItem setting : settings) {
            ...
            switch (setting.type) {
                ...
                case SLIDER_INT:
                    ...
                    float min = setting.min, max = setting.max;
        ...
        if (isBinding && bindingModule != null) renderKeybindPopup(context);
        super.render(context, mouseX, mouseY, delta);
    }
    
    private void renderKeybindPopup(DrawContext context) {
        context.fill(0, 0, width, height, new Color(0, 0, 0, 200).getRGB());
        int boxWidth = 280, boxHeight = 100;
        int boxX = width / 2 - boxWidth / 2, boxY = height / 2 - boxHeight / 2;
                ...
                switch (setting.type) {
                    ...
                    case MODE:
                        ...
                        for (int j = 0; j < modes.length; j++) {
                            if (modes[j].equals(setting.value)) { currentIdx = j; break; }
                        }
                    ...
                    case SLIDER_INT:
                        ...
                        if (mouseX >= sliderX && mouseX <= sliderX + 150) {
                            ...
                            setting.value = (setting.type == SettingType.SLIDER_INT) ? (int)value : value;
                            applySetting(setting, aura);
                            SoundManager.playClickSound();
                        }
                        break;
    ...
    private void applySetting(SettingItem setting, Aura aura) {
        if (aura == null) return;
        switch (setting.name) {
            case "Range": aura.range = (float)setting.value; break;
            case "Through Walls": aura.throughWalls = (boolean)setting.value; break;
            case "APS": aura.aps = (int)setting.value; break;
            case "FOV": aura.fov = (int)setting.value; break;
            case "Rotation Mode":
                String rot = (String)setting.value;
                if (rot.equals("NONE")) aura.rotationMode = Aura.RotationMode.NONE;
                else if (rot.equals("LEGIT")) aura.rotationMode = Aura.RotationMode.LEGIT;
                else if (rot.equals("NORMAL")) aura.rotationMode = Aura.RotationMode.NORMAL;
                break;
            case "Aim Mode":
                String aim = (String)setting.value;
                if (aim.equals("HEAD")) aura.aimMode = Aura.AimMode.HEAD;
                else if (aim.equals("BODY")) aura.aimMode = Aura.AimMode.BODY;
                else if (aim.equals("LEGS")) aura.aimMode = Aura.AimMode.LEGS;
                break;
            case "Smooth Rotation": aura.smoothRotation = (boolean)setting.value; break;
            case "Client Look": aura.clientLook = (boolean)setting.value; break;
            case "Players": aura.players = (boolean)setting.value; break;
            case "Monsters": aura.monsters = (boolean)setting.value; break;
            case "Animals": aura.animals = (boolean)setting.value; break;
            case "Only ESP": aura.onlyESP = (boolean)setting.value; break;
            case "Ignore Invisible": aura.ignoreInvisible = (boolean)setting.value; break;
            case "Ignore Named": aura.ignoreNamed = (boolean)setting.value; break;
            case "Ignore Creative": aura.ignoreCreative = (boolean)setting.value; break;
            case "Ignore Naked": aura.ignoreNaked = (boolean)setting.value; break;
            case "Sort Mode":
                String sort = (String)setting.value;
                if (sort.equals("LowestDistance")) aura.sort = Aura.SortMode.LowestDistance;
                else if (sort.equals("HighestDistance")) aura.sort = Aura.SortMode.HighestDistance;
                else if (sort.equals("LowestHealth")) aura.sort = Aura.SortMode.LowestHealth;
                else if (sort.equals("HighestHealth")) aura.sort = Aura.SortMode.HighestHealth;
                break;
            case "Lock Target": aura.lockTarget = (boolean)setting.value; break;
            case "Pause in Cobweb": aura.pauseInCobweb = (boolean)setting.value; break;
            case "Hit When in Cobweb": aura.hitWhenInCobweb = (boolean)setting.value; break;
            case "Pause in Inventory": aura.pauseInInventory = (boolean)setting.value; break;
            case "Pause While Eating": aura.pauseWhileEating = (boolean)setting.value; break;
            case "Random Hit Delay": aura.randomHitDelay = (boolean)setting.value; break;
            case "Auto Weapon": aura.autoWeapon = (boolean)setting.value; break;
            case "Only Weapon": aura.onlyWeapon = (boolean)setting.value; break;
            case "Shield Breaker": aura.shieldBreaker = (boolean)setting.value; break;
            case "Auto Crit": aura.autoCrit = (boolean)setting.value; break;
            case "Auto Jump": aura.autoJump = (boolean)setting.value; break;
        }
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (isBinding && bindingModule != null) {
            if (keyCode == 256) { isBinding = false; bindingModule = null; }
            else {
                bindingModule.setKeybind(getKeyName(keyCode));
                bindingModule.setKeyCode(keyCode);
                isBinding = false; bindingModule = null;
                SoundManager.playClickSound();
            }
            return true;
        }
        if (isSearching) {
            if (keyCode == 256) { isSearching = false; searchText = ""; return true; }
            if (keyCode == 257 || keyCode == 335) { isSearching = false; return true; }
            if (keyCode == 259) { if (!searchText.isEmpty()) searchText = searchText.substring(0, searchText.length() - 1); return true; }
            String key = getKeyName(keyCode);
            if (key.length() == 1 && (Character.isLetterOrDigit(key.charAt(0)) || key.charAt(0) == ' ')) searchText += key;
            return true;
        }
    ...
    private String getKeyName(int keyCode) {
        switch (keyCode) {
            ...
            case 258: return "TAB"; case 259: return "BACKSPACE"; case 262: return "RIGHT";
            case 263: return "LEFT"; case 264: return "DOWN"; case 265: return "UP";
            default:
    ...
    @Override
    public void close() { super.close(); ConfigManager.saveAllConfigs(); }
    
    private class CategoryItem { String name; int id; CategoryItem(String name, int id) { this.name = name; this.id = id; } }
    private class SubCategoryItem { String name; int id; SubCategoryItem(String name, int id) { this.name = name; this.id = id; } }
    private enum SettingType { TOGGLE, SLIDER, SLIDER_INT, MODE }
    
    private class SettingItem {
        String name; Object value; float min, max; SettingType type; Object extra;
        SettingItem(String name, Object value, SettingType type) { this.name = name; this.value = value; this.type = type; }
        SettingItem(String name, Object value, float min, float max, SettingType type) { this(name, value, type); this.min = min; this.max = max; }
        SettingItem(String name, Object value, int min, int max, SettingType type) { this(name, value, (float)min, (float)max, type); }
        SettingItem(String name, Object value, String[] modes, SettingType type) { this(name, value, type); this.extra = modes; }
    }
}