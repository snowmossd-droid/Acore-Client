...
public class ClickGUI extends Screen {
    ...
    private void renderKeybindPopup(DrawContext context) {
        ...
        context.drawText(textRenderer, "Module: §e" + bindModule.getName(), boxX + 15, boxY + 45, TEXT, true);
        context.drawText(textRenderer, "§7Press any key...", boxX + 15, boxY + 70, TEXT_DIM, true);
        
        // Toggle / Hold options
        context.drawText(textRenderer, "Mode:", boxX + 15, boxY + 95, TEXT, true);
        
        // Toggle button
        int toggleX = boxX + 70;
        context.fill(toggleX, boxY + 90, toggleX + 60, boxY + 108, isHold ? 0xFF555555 : BUTTON_ADD);
        context.drawBorder(toggleX, boxY + 90, 60, 18, BORDER);
        context.drawText(textRenderer, "Toggle", toggleX + 12, boxY + 96, TEXT, true);
        
        // Hold button
        int holdX = boxX + 140;
        context.fill(holdX, boxY + 90, holdX + 50, boxY + 108, isHold ? BUTTON_ADD : 0xFF555555);
        context.drawBorder(holdX, boxY + 90, 50, 18, BORDER);
        context.drawText(textRenderer, "Hold", holdX + 12, boxY + 96, TEXT, true);
        
        context.drawText(textRenderer, "§7Press §eESC §7to cancel", boxX + 15, boxY + 125, TEXT_DIM, true);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isBinding) {
            handleBindPopupClick(mouseX, mouseY);
            return true;
        }
    ...
    private void handleBindPopupClick(double mouseX, double mouseY) {
        int boxWidth = 300;
        int boxHeight = 150;
        int boxX = width / 2 - boxWidth / 2;
        int boxY = height / 2 - boxHeight / 2;
        ...
        // Toggle button
        if (mouseX >= toggleX && mouseX <= toggleX + 60 && mouseY >= boxY + 90 && mouseY <= boxY + 108) {
            isHold = false;
            bindingHold = false;
            SoundManager.playClickSound();
        }
        ...
        // Hold button
        if (mouseX >= holdX && mouseX <= holdX + 50 && mouseY >= boxY + 90 && mouseY <= boxY + 108) {
            isHold = true;
            bindingHold = true;
            SoundManager.playClickSound();
        }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        // Handle Keybind (Middle click popup)
        if (isBinding && bindModule != null) {
            if (keyCode == 256) { // ESC
                isBinding = false;
                bindModule = null;
            ...
            bindModule.setKeybind(keyName);
            bindModule.setKeyCode(keyCode);
            // TODO: Lưu thêm hold mode nếu cần
            isBinding = false;
            bindModule = null;
            isHold = false;
            bindingHold = false;
    ...
    private class ModuleButton {
        ...
        boolean mouseClicked(double mouseX, double mouseY, int button) {
            if (mouseX >= x && mouseX <= x + width && mouseY >= y && mouseY <= y + height) {
                if (button == 0) { // Left click - hiện KeyPin popup
                    pinModule = module;
                    pinInput = "";
                    pinError = "";
                    isEnteringPin = true;
                } else if (button == 2) { // Middle click (chuột giữa) - set keybind
                    bindModule = module;
                    isBinding = true;
                    isHold = false;
                    bindingHold = false;
                }
                // Right click (button == 1) - có thể thêm settings sau
                return true;
            }
            return false;
        }
